...
export interface ChinaRegions {
  name: string;
  regions: Region[];
}

export const chinaRegions: ChinaRegions = {
  name: '国内',
  regions: [
    {
      name: '华东地区',
...
export function getAllRegions(): string[] {
  return chinaRegions.regions.map(r => r.name);
}
...
export function getProvincesByRegion(regionName: string): Province[] {
  const region = chinaRegions.regions.find(r => r.name === regionName);
...
export function getCitiesByProvince(provinceName: string): string[] {
  for (const region of chinaRegions.regions) {
 ...
 * 查找省份所属的地区
 */
export function getRegionByProvince(provinceName: string): string | null {
  for (const region of chinaRegions.regions) {
    const province = region.provinces.find(p => p.name === provinceName);
    if (province) {
      return region.name;
