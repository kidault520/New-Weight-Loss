...
export const salesEvaluationService = {
  async getNotifications(): Promise<EvaluationNotification[]> {
    const { data, error } = await supabase.from('sales_evaluation_notifications').select('*').order('created_at', { ascending: false });
    if (error) throw error;
    return (data || []).map(rowToNotification);
  },

  async getPendingNotifications(): Promise<EvaluationNotification[]> {
    const { data, error } = await supabase.from('sales_evaluation_notifications').select('*').eq('status', 'pending').order('created_at', { ascending: false });
    if (error) throw error;
    return (data || []).map(rowToNotification);
  },

  async getById(id: string): Promise<EvaluationNotification | null> {
    const { data, error } = await supabase.from('sales_evaluation_notifications').select('*').eq('id', id).maybeSingle();
    if (error) throw error;
  ...
  async add(notification: Omit<EvaluationNotification, 'id' | 'createdAt'>): Promise<EvaluationNotification> {
    ...
    const { data, error } = await supabase.from('sales_evaluation_notifications').insert(row).select().single();
    if (error) throw error;
    return rowToNotification(data);
  },

  async addNotifications(notifications: Omit<EvaluationNotification, 'id' | 'createdAt'>[]): Promise<void> {
    const rows = notifications.map((n) => ({
      person_id: n.personId,
      person_name: n.personName ?? null,
      current_rank: n.currentRank ?? null,
      evaluation_period: n.evaluationPeriod,
      evaluation_date: n.evaluationDate ?? null,
      action: n.action,
      target_rank: n.targetRank ?? null,
      reason: n.reason ?? null,
      condition_details: n.conditionDetails ?? [],
      status: n.status ?? 'pending',
    }));
    const { error } = await supabase.from('sales_evaluation_notifications').insert(rows);
    if (error) throw error;
  },

  async updateStatus(id: string, status: NotificationStatus, approvedBy?: string, rejectReason?: string): Promise<EvaluationNotification> {
    const row: Record<string, unknown> = {
      status,
    ...
    const { data, error } = await supabase.from('sales_evaluation_notifications').update(row).eq('id', id).select().single();
    if (error) throw error;
    return rowToNotification(data);
  },

  async addApprovalHistory(history: { notificationId: string; personId: string; personName?: string; action: 'approve' | 'reject'; reason?: string; approvedBy: string }): Promise<void> {
  ...
  async getApprovalHistories(): Promise<{ id: string; notificationId: string; personId: string; personName?: string; action: string; reason?: string; approvedBy: string; approvedAt: string }[]> {
    const { data, error } = await supabase.from('sales_approval_history').select('*').order('approved_at', { ascending: false });
    if (error) throw error;
    return (data || []).map((r) => ({
      id: r.id,
