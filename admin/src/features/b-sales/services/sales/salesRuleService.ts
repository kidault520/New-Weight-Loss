...
export const salesRuleService = {
  async getAll(): Promise<RuleSet[]> {
    const { data, error } = await supabase.from('sales_rule_sets').select('*').order('effective_date', { ascending: false });
    if (error) throw error;
    return (data || []).map(rowToRuleSet);
  },

  async getById(id: string): Promise<RuleSet | null> {
    const { data, error } = await supabase.from('sales_rule_sets').select('*').eq('id', id).maybeSingle();
    if (error) throw error;
    return data ? rowToRuleSet(data) : null;
  },

  async getCurrentRuleSetId(): Promise<string | null> {
    const { data, error } = await supabase.from('sales_current_rule_set').select('rule_set_id').eq('id', FIXED_CONFIG_ID).maybeSingle();
    if (error) throw error;
  ...
  async setCurrentRuleSetId(ruleSetId: string): Promise<void> {
    const { error } = await supabase.from('sales_current_rule_set').upsert({ id: FIXED_CONFIG_ID, rule_set_id: ruleSetId }, { onConflict: 'id' });
    if (error) throw error;
  },
  ...
  async upsert(ruleSet: Partial<RuleSet> & { name: string; version: number; effectiveDate: string }): Promise<RuleSet> {
    ...
    const { data, error } = await supabase.from('sales_rule_sets').upsert(row, { onConflict: 'id' }).select().single();
    if (error) throw error;
