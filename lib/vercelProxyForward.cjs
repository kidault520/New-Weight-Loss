...
function normalizeProxyOrigin(raw) {
  let o = String(raw || '').trim().replace(/\/+$/, '');
  // 常见误配：把 Node 写成了 …/api，会导致转发到 …/api/api/auth/… → 上游 404
  if (o.endsWith('/api')) {
...
async function proxyApiRequest(req, res, pathAfterApi) {
  const origin = normalizeProxyOrigin(process.env.API_PROXY_ORIGIN);
  if (!origin) {
    res.statusCode = 503;
...
module.exports = { proxyApiRequest, normalizeProxyOrigin };
