/*
  # Create Complete Health Tracking Database Schema with Nickname Support

  1. New Tables
    - `user_profiles` - User profile information with nickname and onboarding fields
...
-- User profiles table with nickname and all onboarding fields
...
CREATE TABLE IF NOT EXISTS health_records (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE,
  record_type text NOT NULL CHECK (record_type IN ('weight', 'water', 'steps', 'food', 'exercise', 'measurements', 'calories', 'sleep', 'blood_glucose', 'hrv')),
